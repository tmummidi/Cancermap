 ...
 *
 * per issue 96 we had to rewrite much of this because they want text labels AND icons but that's not how anybody else ever wanted it...
 */
L.Control.BoxZoom = L.Control.extend({
    ...
    onAdd: function (map) {
        ...
        L.DomEvent.addListener(this.controlDiv, 'click', function () {
            this.control.toggleState();
        });

        // hack issue 96, they decided they want a text label and a magnifying glass, which is a bit of a rework from one single DIV with a class
        var button = L.DomUtil.create('i', 'fa fa-search-plus', this.controlDiv);
        var textlabel = L.DomUtil.create('span', 'leaflet-control-boxzoom-text', this.controlDiv);
        textlabel.innerHTML = "Box Zoom";
